...
def lambda_handler(event, context):
    ...
    try:
        # DEBUG: print full event to CloudWatch
        print("DEBUG Incoming event:", json.dumps(event))

        # Parse JSON body from proxy request
        if event.get("body"):
        ...
        if not file_name:
            return {
                'statusCode': 400,
                'headers': cors_headers,
                'body': json.dumps({'error': 'Missing required field: file_name'})
            }
        ...
        if file_size > MAX_FILE_SIZE:
            return {
                'statusCode': 400,
                'headers': cors_headers,
                'body': json.dumps({'error': f'File too large. Max allowed: {MAX_FILE_SIZE / (1024*1024)}MB'})
            }
