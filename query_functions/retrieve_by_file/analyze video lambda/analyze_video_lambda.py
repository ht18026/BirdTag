...
def lambda_handler(event, context):
    ...
    try:
        ...
        try:
            ...
            print(
                f"Analysis completed. Detected {len(detected_species)} species: {list(detected_species.keys()) if detected_species else []}")

            return {
                'detected_species': list(detected_species.keys()) if detected_species else [],
                # Return list of detected bird tags
                'file_type': 'video',
                'filename': filename,
            ...
            try:
                ...
                if detections.tracker_id is not None:
                    ...
                    if processed_count % 20 == 0:
                        ...
                        print(
                            f"Progress: {progress:.1f}% (frame {frame_count}/{total_frames}, processed {processed_count}, time: {elapsed_time:.1f}s)")
...
def get_max_processing_time():
    ...
    """
    return int(os.environ.get('MAX_PROCESSING_TIME',
                              '25'))  # Default 25 seconds, leaving 5 seconds buffer for 30-second API Gateway
...
def get_model_path():
    ...
    if not os.path.exists(model_local_path):
        ...
        bucket = os.environ.get('MODEL_BUCKET', 'birdtag-models-fit5225-g138')
