...
def lambda_handler(event, context):
    ...
    if not isinstance(input_urls, list) or not all(isinstance(u, str) for u in input_urls):
        return {
            'statusCode': 400,
            'headers': CORS_HEADERS,
            'body': json.dumps({'error': "'url' must be a list of strings."})
        }
    ...
    if not isinstance(tags_to_modify_str, list) or not all(isinstance(t, str) for t in tags_to_modify_str):
        return {
            'statusCode': 400,
            'headers': CORS_HEADERS,
            'body': json.dumps({'error': "'tags' must be a list of strings (e.g., ['crow,1'])."})
        }
    ...
    if tag_parsing_errors:
        return {
            'statusCode': 400,
            'headers': CORS_HEADERS,
            'body': json.dumps({'errors': ["Input tag parsing errors:"] + tag_parsing_errors})
        }
    if not parsed_tags and tags_to_modify_str:
        return {
            'statusCode': 400,
            'headers': CORS_HEADERS,
            'body': json.dumps({'error': 'No valid tags to process after parsing.'})
        }
    if not parsed_tags:
        return {
            'statusCode': 200,
            'headers': CORS_HEADERS,
            'body': json.dumps({
                'message': 'No tags specified in the request. No operations performed.',
                'success_operations': [],
                'failed_operations': []
            })
        }
    ...
    if not base_media_info_map:
        return {
            ...
            'headers': CORS_HEADERS,
            'body': json.dumps({
                'error': 'No media records found for the provided URLs.',
                'success_operations': [],
                'failed_operations': []
            })
        }
        ...
        for tag_detail in parsed_tags:
            ...
            try:
                if operation_type == 1:  # ADD
                    ...
                    success_ops_details.append(f"Media ID '{media_id}', Tag '{species}': Count updated by {count_delta} (Add).")

                elif operation_type == 0:  # REMOVE
                    response = dynamodb_client.get_item(TableName=TABLE_NAME, Key=item_key)
                    item_to_remove = response.get('Item')

                    if item_to_remove:
                        current = {k: deserializer.deserialize(v) for k, v in item_to_remove.items()}
                        ...
                        if new_count <= 0:
                            ...
                            success_ops_details.append(f"Media ID '{media_id}', Tag '{species}': Removed (count <= 0).")
                        else:
                            dynamodb_client.update_item(
                                TableName=TABLE_NAME,
                                Key=item_key,
                            ...
                            success_ops_details.append(f"Media ID '{media_id}', Tag '{species}': Count updated to {new_count} (Remove).")
                    else:
                        success_ops_details.append(f"Media ID '{media_id}', Tag '{species}': Not found for removal, ignored.")
